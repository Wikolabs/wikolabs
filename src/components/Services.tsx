...
const OFFER_CARD_BG = [
  /* 0 — Commercial (7) */
  "hsla( 43, 90%, 55%, 0.17)",  // gold
  "hsla(186, 85%, 50%, 0.15)",  // cyan
  "hsla(217, 85%, 60%, 0.17)",  // blue
  "hsla(142, 72%, 46%, 0.15)",  // green
  "hsla(270, 72%, 60%, 0.17)",  // purple
  "hsla(330, 80%, 60%, 0.15)",  // pink
  "hsla( 32, 90%, 55%, 0.17)",  // orange
  /* 1 — Support (4) */
  "hsla(174, 80%, 46%, 0.15)",  // teal
  "hsla(239, 80%, 65%, 0.17)",  // indigo
  "hsla( 83, 76%, 46%, 0.15)",  // lime
  "hsla(200, 85%, 55%, 0.15)",  // sky
  /* 2 — BI (4) */
  "hsla(  0, 80%, 60%, 0.15)",  // red
  ...
  "hsla(222, 82%, 62%, 0.17)",  // cornflower
  "hsla(310, 70%, 58%, 0.15)",  // fuchsia
  /* 4 — Vision (4) */
  "hsla(165, 76%, 46%, 0.15)",  // aqua-green
  "hsla(255, 76%, 63%, 0.17)",  // slate-purple
  "hsla( 35, 88%, 58%, 0.17)",  // amber
  "hsla(195, 82%, 52%, 0.15)",  // cerulean
  /* 5 — Web (4) */
  "hsla(340, 78%, 58%, 0.15)",  // rose
  "hsla(120, 66%, 46%, 0.15)",  // medium green
  "hsla(230, 82%, 63%, 0.17)",  // periwinkle
  "hsla( 56, 82%, 52%, 0.17)",  // golden-yellow
  /* 6 — IoT (4) */
  "hsla( 22, 88%, 55%, 0.17)",  // burnt orange
  "hsla(180, 78%, 48%, 0.15)",  // aqua
  ...
  fr: {
    ...
    categories: [
      { label: "Cycle Commercial & Marketing", offers: [
        ...
        { title: "Agent Sourcing de Leads",       desc: "200 contacts ICP+ collectés, enrichis et écrits dans votre CRM chaque matin. ChatGPT ne touche pas Apollo ni votre base de données." },
        { title: "Agent Outreach Multicanal",     desc: "Séquences email + LinkedIn + Twilio sur 3 canaux, relances automatiques sur ouverture/clic. Un chatbot rédige ; lui envoie, traque et relance." },
        { title: "Agent Qualification BANT",      desc: "Il lit la réponse du prospect, calcule le score et route : HOT → setter / COLD → nurturing. Un chatbot répond ; lui décide et agit." },
        { title: "Agent Setter (Prise de RDV)",   desc: "Créneau proposé depuis votre vrai agenda Calendly, confirmé, rappel J-1 envoyé. Le chatbot suggère ; lui réserve pour de vrai." },
        { title: "Agent Proposition Commerciale", desc: "Notes du discovery call → proposition PDF → signature DocuSign. Le chatbot s'arrête au brouillon ; lui va jusqu'au contrat signé." },
        { title: "Agent Rétention & Upsell",      desc: "Analyse le comportement réel dans vos données, détecte le churn risk, déclenche la séquence. Un chatbot ne surveille rien en continu." },
      ]},
      { label: "Support & Service Client", offers: [
        { title: "Agent SAV E-commerce (RAG)",    desc: "Répond sur le vrai statut de commande WooCommerce/Shopify par WhatsApp, 24h/24. ChatGPT ne connaît pas vos commandes ; lui les lit en direct." },
        { title: "Agent Triage & Escalade",       desc: "Classe par urgence, répond aux FAQ, et sait quand passer la main à un humain. Un chatbot répond à tout ; lui sait quand ne pas répondre." },
        { title: "Agent Onboarding Client",       desc: "Checklist d'onboarding suivie dans Notion/Jira, relances si une étape manque. Le chatbot ne suit rien dans le temps ; lui oui." },
        { title: "Agent Notes Discovery",         desc: "Transcrit l'appel (Whisper), structure et crée la fiche CRM automatiquement. ChatGPT sort du texte ; lui l'écrit dans votre système." },
      ]},
      { label: "BI & Intelligence Décisionnelle", offers: [
        { title: "Agent BI Langage Naturel",      desc: "Question en français → SQL → dashboard livré. Il interroge vos vraies données BigQuery, pas des exemples génériques." },
      ...
      { label: "Recherche & Données Structurées", offers: [
        ...
        { title: "Recommandation Personnalisée",  desc: "Collaborative filtering sur vos vraies données d'achat : le bon produit, à la bonne personne, au bon moment. Pas de fiction générique." },
        { title: "Pipeline de Données Intelligent",desc: "ETL, CDC, sync temps réel et data warehouse interrogeable en langage naturel. Strictement hors portée d'un LLM conversationnel." },
        { title: "Fine-tuning & MLOps",           desc: "Votre propre modèle entraîné, surveillé et mis à jour en production. Un chatbot est un produit fini ; lui en construit un sur mesure." },
      ]},
      { label: "Documents, Vision & Industrie", offers: [
        { title: "OCR & Extraction Documentaire", desc: "Email reçu → données extraites → écrites dans votre ERP, < 0,3% d'erreur. Le chatbot lit une facture ; lui en traite 2 000/mois automatiquement." },
        { title: "Vision IA & Détection d'Objets",desc: "Détection à 40 images/s sur vos flux industriels en continu. Un chatbot ne traite pas un flux vidéo industriel." },
        { title: "BIM & Point Cloud",             desc: "Données nuage de points et maquettes 3D lourdes traitées en pipeline automatisé. Strictement hors capacité d'un LLM grand public." },
        { title: "Cartographie & Géolocalisation",desc: "Routage spatial calculé sur vos vrais points GPS logistiques. Le chatbot ne calcule pas d'itinéraires optimisés sur vos données terrain." },
      ]},
      ...
      { label: "IoT & IA Embarquée", offers: [
        { title: "IA Embarquée sur Edge Device",  desc: "Modèle IA local sur Raspberry Pi/Jetson, < 50ms, zéro cloud requis. Un chatbot vit dans le cloud ; lui tourne sur l'appareil sans internet." },
        { title: "Monitoring IoT Temps Réel",     desc: "Capteurs supervisés en flux continu (Kafka/Grafana), anomalie détectée en temps réel. Le chatbot ne se branche pas sur vos équipements." },
        { title: "Vision Machine sur Caméra IP",  desc: "Contrôle qualité directement sur caméra IP, traitement local, alerte instantanée. Totalement hors d'un LLM conversationnel." },
        { title: "Maintenance Prédictive IA",     desc: "Prédiction de panne 7–30 jours à l'avance sur vibrations/thermique/électrique. Le chatbot ne lit pas vos capteurs." },
      ]},
    ],
  },
  en: {
    ...
    categories: [
      { label: "Commercial Cycle & Marketing", offers: [
        ...
        { title: "Lead Sourcing Agent",           desc: "200 ICP+ contacts collected, enriched and written into your CRM every morning. ChatGPT can't touch Apollo or your database." },
        { title: "Multichannel Outreach Agent",   desc: "Email + LinkedIn + Twilio sequences on 3 channels, auto follow-ups on open/click. A chatbot drafts; this one sends, tracks and follows up." },
        { title: "BANT Qualification Agent",      desc: "Reads the prospect reply, scores it, routes: HOT → setter / COLD → nurturing. A chatbot answers; this one decides and acts." },
        { title: "Setter Agent (Meeting Booking)",desc: "Real Calendly slot proposed, confirmed, D-1 reminder sent automatically. A chatbot suggests; this one actually books." },
        { title: "Commercial Proposal Agent",     desc: "Discovery notes → PDF proposal → DocuSign signature. The chatbot stops at the draft; this one goes to the signed contract." },
        { title: "Retention & Upsell Agent",      desc: "Analyzes real behavioral data, detects churn risk, triggers the re-engagement sequence. A chatbot monitors nothing continuously." },
      ]},
      { label: "Support & Customer Service", offers: [
        { title: "E-commerce SAV Agent (RAG)",    desc: "Answers on real WooCommerce/Shopify order status via WhatsApp, 24/7. ChatGPT doesn't know your orders; this one reads them live." },
        { title: "Triage & Escalation Agent",     desc: "Sorts by urgency, answers FAQs, knows when to hand off to a human. A chatbot answers everything; this one knows when not to." },
        { title: "Client Onboarding Agent",       desc: "Onboarding checklist tracked in Notion/Jira, auto-reminders when a step is missing. A chatbot tracks nothing over time; this one does." },
        { title: "Discovery Notes Agent",         desc: "Transcribes the call (Whisper), structures it, auto-creates the CRM record. ChatGPT outputs text; this one writes it into your system." },
      ]},
      { label: "BI & Decision Intelligence", offers: [
        { title: "Natural Language BI Agent",     desc: "Question in English → SQL → dashboard delivered. It queries your real BigQuery data, not generic examples." },
      ...
      { label: "Search & Structured Data", offers: [
        ...
        { title: "Personalized Recommendation",   desc: "Collaborative filtering on your real purchase data: the right product, to the right person, at the right moment. No generic fiction." },
        { title: "Intelligent Data Pipeline",     desc: "ETL, CDC, real-time sync and data warehouse queryable in plain language. Strictly beyond any conversational LLM." },
        { title: "Fine-tuning & MLOps",           desc: "Your own model trained, monitored and updated in production. A chatbot is a finished product; this one builds one from scratch." },
      ]},
      { label: "Documents, Vision & Industry", offers: [
        { title: "OCR & Document Extraction",     desc: "Email received → data extracted → written into your ERP, < 0.3% error. A chatbot reads one invoice; this one processes 2,000/month automatically." },
        { title: "Vision AI & Object Detection",  desc: "Detection at 40 frames/s on your industrial streams, continuously. A chatbot doesn't process industrial video feeds." },
        { title: "BIM & Point Cloud",             desc: "Heavy 3D point cloud and BIM data processed in automated pipeline. Strictly beyond any public LLM." },
        { title: "Geolocation & Mapping",         desc: "Spatial routing calculated on your real GPS logistics data. A chatbot doesn't compute optimized routes on your field data." },
      ]},
      ...
      { label: "IoT & Edge AI", offers: [
        { title: "Edge AI on Embedded Devices",   desc: "Lightweight AI model on Raspberry Pi/Jetson, < 50ms, zero cloud required. A chatbot lives in the cloud; this one runs on-device without internet." },
        { title: "Real-time IoT Monitoring",      desc: "Sensors supervised in real-time streams (Kafka/Grafana), anomaly detected instantly. A chatbot doesn't connect to your equipment." },
        { title: "Machine Vision on IP Camera",   desc: "Quality control directly on IP cameras, local processing, instant alerts. Completely beyond any conversational LLM." },
        { title: "AI Predictive Maintenance",     desc: "Failure prediction 7–30 days ahead on vibration/thermal/electrical data. A chatbot doesn't read your sensors." },
      ]},
    ],
  },
};
...
export default function Services({
  ...
  interface SearchMsg { role: "user" | "assistant"; text: string; matchIds?: string[]; streaming?: boolean; }
  const [chatMsgs, setChatMsgs] = useState<SearchMsg[]>([]);
  const [chatInput, setChatInput] = useState("");
  const [chatLoading, setChatLoading] = useState(false);
  const [currentMatchIds, setCurrentMatchIds] = useState<string[] | null>(null);
  const chatEndRef = useRef<HTMLDivElement>(null);
  const chatInputRef = useRef<HTMLInputElement>(null);
  const chatInited = useRef(false);

  useEffect(() => {
    if (chatInited.current) return;
    chatInited.current = true;
    const welcome = lang === "fr"
      ? "Bonjour ! Décrivez votre besoin en quelques mots — je vous recommande les solutions Wikolabs les plus adaptées. Vous pouvez affiner au fil de la conversation."
      : "Hello! Describe your need in a few words — I'll recommend the most relevant Wikolabs solutions. You can refine as we go.";
    setChatMsgs([{ role: "assistant", text: welcome }]);
  ...
  const handleChatSend = async () => {
    ...
    setChatInput("");
    const newMsgs: SearchMsg[] = [...chatMsgs, { role: "user", text: userText }];
    setChatMsgs([...newMsgs, { role: "assistant", text: "", streaming: true }]);
    setChatLoading(true);
    scrollChat();

    const catalog = buildOfferCatalog(lang);
    const systemPrompt = lang === "fr"
      ? `Tu es l'assistant de recherche de Wikolabs. Tu aides les utilisateurs à identifier les bons services IA/data.\n\nCatalogue (ID | Titre | Catégorie) :\n${catalog}\n\nL'utilisateur peut envoyer plusieurs messages pour affiner. Utilise l'historique complet.\nRéponds UNIQUEMENT avec un JSON valide, sans markdown :\n{"summary":"2-3 phrases en français expliquant ta compréhension et les services recommandés","matchIds":["0-0","1-2"]}\nRègles : 2 à 8 services max ; JSON brut uniquement.`
      : `You are the Wikolabs search assistant. Help users find the right AI/data services.\n\nCatalog (ID | Title | Category):\n${catalog}\n\nThe user may send follow-up messages to refine. Use the full conversation history.\nRespond ONLY with valid JSON, no markdown:\n{"summary":"2-3 sentences in English explaining your understanding and recommended services","matchIds":["0-0","1-2"]}\nRules: 2 to 8 services max; raw JSON only.`;

    // Build API history from previous messages (exclude welcome, convert assistant msgs back to JSON string)
    const apiMsgs = newMsgs.slice(1).map(m => ({
      role: m.role as "user" | "assistant",
      content: m.role === "assistant"
        ? JSON.stringify({ summary: m.text, matchIds: m.matchIds ?? [] })
        : m.text,
    }));
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: apiMsgs, systemPrompt }),
      });
      ...
      if (parsed?.summary && Array.isArray(parsed.matchIds)) {
        setChatMsgs([...newMsgs, { role: "assistant", text: parsed.summary, matchIds: parsed.matchIds }]);
        setCurrentMatchIds(parsed.matchIds);
      } else throw new Error();
    } catch {
      setChatMsgs([...newMsgs, { role: "assistant", text: t.searchError }]);
    } finally {
  ...
  return (
    <section id="services" className={styles.services}>
      ...
      {/* ── Chat Search Panel ── */}
      <div className={`reveal d3 ${styles.chatSearchPanel}`}>
        <div className={styles.chatSearchHeader}>
          <div className={styles.chatSearchAvatar}><LuBot size={15} /></div>
          <div>
            <div className={styles.chatSearchTitle}>{lang === "fr" ? "Recherche guidée par IA" : "AI-guided search"}</div>
            <div className={styles.chatSearchSub}>{lang === "fr" ? "Affinez au fil de la conversation" : "Refine as you chat"}</div>
          </div>
        </div>

        <div className={styles.chatSearchMessages}>
          {chatMsgs.map((msg, i) => (
            <div key={i} className={`${styles.chatMsg} ${msg.role === "user" ? styles.chatMsgUser : styles.chatMsgBot}`}>
              {msg.role === "assistant" && (
                <div className={styles.chatMsgAvatar}><LuBot size={11} /></div>
              )}
              <div className={`${styles.chatMsgBubble} ${msg.streaming ? styles.chatMsgTyping : ""}`}>
                {msg.streaming ? <><span /><span /><span /></> : msg.text}
              </div>
            </div>
          ))}
          <div ref={chatEndRef} />
        ...
        <div className={styles.chatSearchInputRow}>
          <input
            ref={chatInputRef}
            className={styles.chatSearchInput}
            type="text"
            placeholder={chatLoading ? t.searching : t.searchPlaceholder}
            value={chatInput}
          ...
          />
          <button
            className={styles.chatSearchSendBtn}
            onClick={handleChatSend}
            disabled={chatLoading || !chatInput.trim()}
            aria-label={t.searchBtn}
          >
            {chatLoading ? <span className={styles.chatSearchSpinner} /> : <LuSend size={14} />}
          </button>
        </div>
      </div>

      {/* ── Matched results from chat ── */}
      {currentMatchIds && currentMatchIds.length > 0 && (
        <div className={styles.searchResults}>
          <div className={styles.searchResultsHeader}>
            <span className={styles.searchResultsLine} />
            <span className={styles.searchResultsLabel}>{lang === "fr" ? "Suggestions" : "Suggestions"}</span>
            <span className={styles.searchResultsLine} />
          </div>
          <div className={styles.searchGrid}>
            {currentMatchIds.map(id => {
